...
__all__ = ['match_lbs', 'TextLearner', 'text_classifier_learner']
...
# Cell
def match_lbs(
    old_wgts:dict, # Embedding weights of labels in colab model
    old_vocab:dict, # Vocabulary of labels used for colab pre-training
...
class TextLearner(Learner):
    ...
    def load_lbs(self,
        ...
        lbs_fname:str, # Saved Vocabulary of collab labels in pickle format
        device:(int,str,torch.device)=None # Device used to load, defaults to `dls` device
    ):
        ...
        wgts = torch.load(wgts_fname, map_location= lambda storage,loc: storage)
        if 'model' in wgts: wgts = wgts['model'] #Just in case the pretrained model was saved with an optimizer
        wgts = match_lbs(wgts, old_lbs_vocab, new_lbs_vocab)
